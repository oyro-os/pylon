...
use pylon::app::{AppManager, sql::SqlAppManager};
...
async fn postgres_resolves_by_id_and_key_and_filters_disabled() {
    ...
    let setup = AnyPoolOptions::new().max_connections(2).connect(&url()).await
        .expect("connect Postgres (is pylon-test-postgres up on 5433?)");
    sqlx::query(DDL).execute(&setup).await.expect("create table");
    ...
    sqlx::query(ins).bind(&id).bind(&key).bind("sec").bind("P").bind(7_i64).bind(1_i64).bind(1_i64).bind(1_i64)
        .bind("[{\"url\":\"https://e.test\",\"event_types\":[\"channel_occupied\"]}]")
        .execute(&setup).await.unwrap();
    sqlx::query(ins).bind(&off_id).bind(&off_key).bind("s").bind("Off").bind(0_i64).bind(0_i64).bind(0_i64).bind(0_i64).bind("[]")
        .execute(&setup).await.unwrap();
